	...
	var App = function (_React$Component) {
	  ...
	  _createClass(App, [{
	    ...
	    value: function render() {
	      return _react2.default.createElement("div", { id: "react-root" });
	    }
	  }]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	function updateOptions(inst, multiple, propValue) {
	  var selectedValue, i;
	  var options = ReactMount.getNode(inst._rootNodeID).props;
	  ...
	  if (multiple) {
	    ...
	    for (i = 0; i < props.length; i++) {
	      var selected = selectedValue.hasOwnProperty(props[i].value);
	      if (props[i].selected !== selected) {
	        props[i].selected = selected;
	      }
	    }
	  } else {
	    ...
	    for (i = 0; i < props.length; i++) {
	      if (props[i].value === selectedValue) {
	        props[i].selected = true;
	        return;
	      }
	    }
	    if (props.length) {
	      props[0].selected = true;
	    }
	  }
	}
	 ...
	 * selected option will trigger updates to the rendered props.
	 *
	 * If it is supplied (and not null/undefined), the rendered props will not
	 * update in response to user actions. Instead, the `value` prop must change in
	 * order for the rendered props to update.
	 *
	 * If `defaultValue` is provided, any props with the supplied values will be
	 * selected.
	 */
	var ReactDOMSelect = {
